...
document.querySelectorAll('a[href^="#"]').forEach(function (link) {
  link.addEventListener("click", function (e) {
    ...
    //
    // Виняток - бічне меню розділів у кейсі. Там людина стрибає між
    // сусідніми абзацами однієї сторінки, і шторка на таку коротку
    // відстань збиває з пантелику: після неї незрозуміло, куди саме
    // тебе перенесло. Туди просто гортаємо, щоб рух було видно.
    var уМеню = !!(link.closest && link.closest(".toc"));
    if (!уМеню && window.pageVeil && window.pageVeil.cover(function () { goTo(true); })) return;
    goTo(false);
  });
});
...
(function () {
  ...
  // Лічильник під кінець пригальмовує: до 85% кроки великі, далі дрібні -
  // остання ділянка тягнеться, як у справжньому завантаженні. Крок 40мс і
  // більші приростки: разом близько 0.6 с. Було 70мс і 2.0 с - людина, яка
  // прийшла вперше, дві секунди дивилась на лічильник замість сайту, і це
  // єдине, що вона встигала побачити, перш ніж закрити вкладку. Ефект
  // завантаження лишається, але вже не коштує уваги.
  var value = 0;
