...
// Обкладинка кейсу, кращий файл якої може ще не лежати в теці. Пробуємо
// завантажити його окремо і підставляємо, лише коли він справді є: так на
// сторінці ніколи не буває битої картинки, а щойно файл покладено -
// обкладинка міняється сама, без правок у розмітці.
(function () {
  Array.prototype.forEach.call(document.querySelectorAll("img[data-upgrade]"), function (img) {
    var шлях = img.getAttribute("data-upgrade");
    if (!шлях) return;
    var проба = new Image();
    проба.onload = function () { img.src = шлях; };
    проба.src = шлях;
  });
})();
